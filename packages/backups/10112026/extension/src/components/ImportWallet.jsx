  ...
  useEffect(() => {
    const checkPassword = async () => {
      try {
        ...
        if (protectionIsOff) {
          // Protection is OFF = no valid password exists, create new
          setExistingPasswordDetected(false);
          logger.log('[ImportWallet] Protection is OFF - will create new password');
          return;
        }
        ...
        // Only show verify if password exists AND wallets exist
        const passwordExists = has || hasLocalAuth;
        setExistingPasswordDetected(passwordExists && !isEmpty);
        logger.log('[ImportWallet] Password check - service:', has, 'localStorage:', hasLocalAuth, 'wallets:', !isEmpty);
      } catch (e) {
