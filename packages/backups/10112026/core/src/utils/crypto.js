// Ed25519 implementation for X1/Solana
// Based on TweetNaCl with proper mnemonic support
...
export async function generateKeyPair(seed) {
  const d = new Uint8Array(await crypto.subtle.digest('SHA-512', seed));
  d[0] &= 248;
  d[31] &= 127;
  d[31] |= 64;
...
export async function sign(message, secretKey) {
  ...
  const pk = secretKey.subarray(32);
